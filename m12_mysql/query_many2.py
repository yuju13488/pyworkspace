...
try:
    ...
    query="SELECT ename,hiredate,salary FROM employee WHERE deptno=%(deptno)s AND title=%(title)s" #動態找資料，固定為%s
    deptno=100
    title='engineer'
    cursor.execute(query,{'deptno':deptno,'title':title}) #dictionary無序列
